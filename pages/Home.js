...
import { allMarkersAtom } from "../recoil/state";
import { useRecoilValue } from "recoil";

export default function Home() {
  const [showPinThread, setShowPinThread] = useState(false)
  const [selectedThreads, setSelectedThreads] = useState([])
  const [markers, setMarkers] = useRecoilValue(allMarkersAtom);
  ...
  function renderRandomMarkers(n) {
    const {latitude, longitude, latitudeDelta, longitudeDelta} = initialRegion.mapRegion;
